...
3. **身份/痕迹禁用词 0 命中（硬）**：全文（含表格）扫描 `FORBIDDEN_WORDS`（skill / WorkBuddy / 智能体 / 合并 / 融合两 / 两套解 / 底版 / 参考解 …），命中即拒；竞赛要求的 AI 工具使用声明（按 `文档/七轮自审框架.md 附录 A`）不受此项拦截。口语主语词（我们/本文/该模型）**不是硬闸门**，属软规则，由去AI味指南在写作阶段约束。
