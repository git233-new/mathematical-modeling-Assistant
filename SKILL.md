...
| Step | 开始前必须读取 | 唯一产出 |
|---|---|---|
| Step 0 开跑自检 | `文档/样式统一规定.md`、`文档/论文合规与自审.md` | 自检通过结论（以 `self_check.py` 退出码为准） |
...
| Step 7 写论文 | `文档/论文写作.md`（「章节写法路由表」）与 `文档/论文合规与自审.md` 动笔前读；写作范式按路由表写对应章节前读（摘要→`文档/摘要写作范式.md`、问题重述与分析→`文档/问题重述与分析写作.md`、假设与符号→`文档/假设与符号写作.md`） | `完整论文.tex` + `完整论文.docx` |
...
3. **身份/痕迹禁用词 0 命中（硬）**：全文（含表格）扫描 `FORBIDDEN_WORDS`（skill / WorkBuddy / 智能体 / 合并 / 融合两 / 两套解 / 底版 / 参考解 …），命中即拒；竞赛要求的 AI 工具使用声明（按 `知识库/写作增强/七轮自审框架.md 附录 A`）不受此项拦截。口语主语词（我们/本文/该模型）**不是硬闸门**，属软规则，由去AI味指南在写作阶段约束。
...
> 本文件 6 条为顶层分类入口；完整 H1–H13 / W1–W5 / D1–D4 清单见 `文档/图片闸门配置与绘图规范.md`，validator 直连该文件。
...
- **建库学习**：发优秀论文 PDF → 按 `tools/paperingest/TOOLGUIDE.md` 解析沉淀到 `知识库/优秀论文案例/`，供后续建模参考；新增论文后再跑 `python tools/paperingest/distill.py --raw <PDF目录>` 刷新全库模板与创新点蒸馏（`知识库/写作增强/获奖论文模板与创新点.md`）。
...
7. **生成论文**：
   - **强制读取**：按「强制读取协议」Step 7 行执行，每章动笔前确认已读对应文件；全文措辞按 `知识库/写作增强/去AI味指南.md` 约束；评审锚点与题型画像按 `知识库/评审增强/国赛评审标准.md`、`知识库/方法库/问题分类.md`
   ...
   - **图表必须三件套（硬规则，违反即预警）**：每张图/表前必须有引导句（`body('如图 N 所示，…')`），后必须有解释段（`body('由图/表 N 可知，…')`）。禁止裸插图表。示例：
     ...
     ```
   - **模型评价与改进必须分点（硬规则）**：7.1 优点 3-5 条、7.2 局限与改进 2-4 条，每条 1-2 句指向具体设计/边界，禁止大段叙述。示例：
     ```python
     ...
     ```
   - **模型假设必须短句编号（硬规则）**：格式 `假设 N（短标题）：具体内容。` 每条 1-2 句，禁止假设后跟长段解释/研究意义/口号式合理性。示例：
     ```python
...
| 论文写法 / 去AI味 | `文档/论文写作.md「章节写法路由表」`（先读路由表，按表逐章读对应文件）→ `文档/摘要写作范式.md`、`文档/问题重述与分析写作.md`、`文档/假设与符号写作.md`、`知识库/写作增强/获奖论文模板与创新点.md`（全库蒸馏的章节骨架与创新点模式，定骨架与检验计划） → `知识库/写作增强/去AI味指南.md`（含 25 通用 + 15 数模专用模式，6-Pass 写作流程）、`知识库/写作增强/七轮自审框架.md`（含 42 项反模式清单）、`知识库/评审增强/国赛评审标准.md`、`知识库/方法库/问题分类.md`、`知识库/方法库/质检清单.md` |
...
| 出图（全中文） | `tools/figure/TOOLGUIDE.md`（绘图运行链、模板、中文样式、选图论证与避坑清单、自检入口）+ `文档/图片闸门配置与绘图规范.md`（H1–H13 / W1–W5 / D1–D4 唯一权威）+ `知识库/方法库/图表规范.md` |
| 论文生成 | `tools/docx/TOOLGUIDE.md`（生成链路、结构校验、LaTeX 导出、可重建入口） |
| 竞赛合规 | `文档/论文合规与自审.md`（执行规范）；当届官方文件（最终依据） |
