---
name: modex
description: 数学建模竞赛高级队友 Skill。模拟高水平建模队伍，从赛题一路产出原创解题详解、可运行综合代码、真实结果图表、DOCX 论文及评审风险分析。用户说"解答题目 / 解题 / 做这道题 / 开做"配合赛题即触发完整解题流程（Step 1→9 自动出论文）。
---
...
| Step 7 写论文 | `文档/论文写作.md`（§二路由表）、`知识库/写作增强/摘要写作范式.md`、`知识库/写作增强/问题重述与分析写作.md`、`知识库/写作增强/假设与符号写作.md`、`知识库/写作增强/去AI味指南.md`、`知识库/评审增强/国赛评审标准.md`、`知识库/方法库/问题分类.md` |
...
- **完整解题（默认）**：发赛题 PDF/文本，并说 **"解答题目 / 解题 / 做这道题 / 开做"** 中任意一个 → 立即自动跑完 Step 1→9，直到产出 DOCX 论文和评审文件。**在本 skill 中，"解题" = "产出完整论文"，不等于"只分析"。**
...
## 完整执行流程（Step 0 → 9，必须自动走完，直到产出终稿论文）
...
| 论文生成 | `tools/docx/` |
| 竞赛合规 | `知识库/写作增强/七轮自审框架.md`（核验，含附录 A 官方 AI 规定原文） |
