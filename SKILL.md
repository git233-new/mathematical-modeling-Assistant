...
8. **结果真实可复现（Step 6 只执行不重复规则）**：论文关键数字、图表、摘要结论必须来自当前批次 `results/` 的真实运行产物（Python 脚本或 SPSS 等人工工具导出文件）；摘要数值逐个溯源（`_fabricated_number_issues` 硬闸门，无来源即拒存）。SPSS 等人工工具统计量与 Python 结果平级，同受健全性检查约束。用不到 SPSS 不建该文件即放行，不设全局强制。
