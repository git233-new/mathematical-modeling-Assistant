...
3. **文献检索（必做，参考文献真实性的唯一来源）**：每题必做，检索范围按题自适应——围绕赛题的理论依据、方法来源与公开数据背景确定查询词，不为凑数堆文献。用 `tools/paper_search/` 执行：OpenAlex 发现候选 → Crossref 核验书目信息，仅 `citation_ready=true` 条目进入参考文献；结果登记 `results/数据/文献检索.json`（铁律 6，检索后必须落盘登记），正文引用文献观点前须核对原文。它与案例库检索（Step 2/4 的 case_retrieval）用途不同——案例库只学方法不产生引用，文献检索产生正式引用。
