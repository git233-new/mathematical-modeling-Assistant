...
- **按题启用**：Champion/Challenger Tournament、OCR、SPSS、复杂敏感性分析仅在题目、数据或用户要求需要时启用；不为满足流程形式制造无效产物。文献检索不在此列——它是每题必做的 Step 3。
...
| Step 3 文献检索 | `tools/paper_search/TOOLGUIDE.md` | `results/数据/文献检索.json` |
...
- **建库学习**：发优秀论文 PDF → 按 `tools/paperingest/TOOLGUIDE.md` 解析沉淀到 `知识库/优秀论文案例/`，供后续建模参考；新增论文后再跑 `python tools/paperingest/distill.py --raw <PDF目录>` 刷新全库模板与创新点统计（结果打印到 stdout，不再落盘 md）。
...
3. **文献检索（必做，参考文献真实性的唯一来源）**：每题必做，检索范围按题自适应——围绕赛题的理论依据、方法来源与公开数据背景确定查询词，不为凑数堆文献。**按 `tools/paper_search/TOOLGUIDE.md` 的流程执行检索、核验与引用门禁**，结果登记 `results/数据/文献检索.json`（铁律 6；文件为列表，每次检索追加一条记录，禁止覆盖）。它与案例库检索（Step 2/4 的 case_retrieval）用途不同：案例库只学方法不产生引用，文献检索产生正式引用。
...
8. **数学验证（论文生成后、评审前）**：独立检查已生成论文与真实结果的一致性（公式-符号/数值-来源/跨段落/图表-正文/假设-检验/Model Contract 验收/参考文献-文献检索.json 对应/LaTeX 完整性/pandoc 产物/ctex 剥离），逐项核对，不通过回 Step 7 修正。此步不改论文不重新建模，只查一致性。
...
| 赛题附件解析 | `tools/docx/TOOLGUIDE.md`（DOCX 解析入口：`ingest.py` 读取原生文本、表格和旧式对象；赛题 PDF/DOCX 不启用 OCR） |
