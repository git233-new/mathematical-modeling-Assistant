...
- **按题启用**：文献检索、Champion/Challenger Tournament、OCR、SPSS、复杂敏感性分析仅在题目、数据或用户要求需要时启用；不为满足流程形式制造无效产物。
...
| Step 7 写论文 | `文档/论文写作.md`（§二路由表）、`文档/论文合规与自审.md`、按需读取写作范式与案例库 |
...
3. **文献检索（按题启用，参考文献真实性的唯一来源）**：当论文需要理论依据、方法来源、公开数据背景或正式参考文献时执行；纯计算复核、只分析或用户明确不需要文献时跳过，并在评审记录中注明原因。启用时**按 `tools/paper_search/TOOLGUIDE.md` 执行检索、核验与引用门禁**，结果追加登记 `results/数据/文献检索.json`（文件为列表，禁止覆盖）。案例库检索只学方法，不产生正式引用。
...
7. **生成论文**：
   - **强制读取**（见强制读取协议）：写论文前**依次 Read 以下文件**，全部加载到上下文后再动笔：① `文档/论文写作.md`（§二路由表确定每章写法来源）→ ② `知识库/写作增强/摘要写作范式.md`（写摘要时读）→ ③ `知识库/写作增强/问题重述与分析写作.md`（写问题重述与分析时读）→ ④ `知识库/写作增强/假设与符号写作.md`（写假设与符号时读）→ ⑤ `知识库/写作增强/去AI味指南.md`（全文写作时读）→ ⑥ `知识库/评审增强/国赛评审标准.md`（评审维度与评分锚点）→ ⑦ `知识库/方法库/问题分类.md`（题型画像判定）。每章动笔前必须确认已读对应文件
...
| 论文写法 / 去AI味 | `文档/论文写作.md §二路由表`（先读路由表，按表逐章读对应文件）→ `知识库/写作增强/摘要写作范式.md`、`知识库/写作增强/问题重述与分析写作.md`、`知识库/写作增强/假设与符号写作.md`、`知识库/写作增强/获奖论文模板与创新点.md`（全库蒸馏的章节骨架与创新点模式，定骨架与检验计划） → `知识库/写作增强/去AI味指南.md`（含 25 通用 + 15 数模专用模式，6-Pass 写作流程）、`知识库/写作增强/七轮自审框架.md`（含 42 项反模式清单）、`知识库/评审增强/国赛评审标准.md`、`知识库/方法库/问题分类.md`、`知识库/方法库/质检清单.md` |
