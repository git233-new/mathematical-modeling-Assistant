...
本 Skill 只维护一条真实运行链：**题目附件 → Problem Card → Model Contract → 代码运行 → `results/` 产物 → 论文**。任何数字、图表和结论不能越过这条链直接进入论文。
...
- **硬规则**：代码闸门、`schemas/` 契约、交付目录边界；违反即停止当前阶段并修复。
...
7. **硬软分层（取代模糊仲裁）**：规则分两层，不存在"谁更严谁赢"的灰色地带。**硬闸门** = `交付硬闸门`节列出的 6 条（篇幅版式/黑色字体/禁用词/三线表/建模公式/结果版式），机器校验、不通过即拒绝保存，无例外。**机检写作规范**（图表上下文/评价章编号/文献取向/代码风格等可静态检查项）以"预警："级输出，与硬闸门一样**必须清零才能保存**，修复路径可含人工确认与 brownfield 降级。**纯写作软规则**（无法机检的措辞/文风项）只在评审报告中扣分，不进机器校验。两者冲突时以硬闸门为准；机检项口径以代码常量为准（如 `FORBIDDEN_WORDS`、`contest_profile.py`）。
8. **结果真实可复现（Step 6 只执行不重复规则）**：论文关键数字、图表、摘要结论必须来自当前批次 `results/` 的真实运行产物（Python 脚本或 SPSS 等人工工具导出文件）；摘要与五至七章数值逐个溯源（含双值推导比对，无来源即拒存）。SPSS 等人工工具统计量与 Python 结果平级，同受健全性检查约束。用不到 SPSS 不建该文件即放行，不设全局强制。
...
| Step 6 真实运行与落盘 | `results/` 图片与数据 | `文档/代码规范.md`（健全性检查 / 失败恢复链） |
...
1. **读题与附件**：枚举 `PROJECT_ROOT/files/` 的赛题 PDF/DOCX 与全部附件，`tools.project_ops.case_retrieval.load_input_bundle()` 全量读取；DOCX 附件的全文/表格/对象清单进入题目分析，WMF/EMF、VML、OLE 公式用 `tools/docx/scripts/extract_docx_content.py` 提取并逐项视觉检查；全程不启用 OCR。**填表类赛题只在 `files/` 原表副本上填值**（保留行列/表头/合并单元格），结果写 `results/数据/`，绝不回写 `files/`；临时资产读后即删。
2. **赛题分析（Problem Card）**：`case_retrieval.py --query-file <赛题>` 检索优秀论文案例，匹配依据只进会话上下文。按 `schemas/problem_card.json` 逐小问填写 Problem Card（变量/约束/目标/数据规模/不确定性/动态性），据此推导候选模型族（设计走"变量→约束→目标→数据结构→数学结构→模型族→算法"，题型与算法不直连）；读题后逐条执行 `知识库/建模通用规范.md`「题意理解红线」。
3. **文献检索（按需触发，参考文献真实性的唯一来源）**：用 `tools/paper_search/scripts/hybrid_scholar.py` 检索并真实核验，通过条目逐条登记 `results/数据/文献检索.csv`（utf-8-sig）；只放行 `citation_ready=true`，禁止虚构。中文文献等后端命不中的走人工核验登记：`hybrid_scholar.py --manual <输入CSV> --project <项目>`（`--template` 出模板，DOI 反查回填），核验责任在人。
...
7. **生成论文**：按规范入口读 `文档/论文写作.md`，按「章节写法路由表」写某一章前才读该章范式；章源 `.paper_work/NN_*.md`，整章一次性写入、同一章只写一次，无逐章断点。执行链：`pf.preflight_check(outline)` 预检 → 逐章写入（动笔前读对应范式与去AI味指南）→ **全量自检清零**（`validate_paper_structure` 硬错误+预警全修复、重建重跑至 0 issue）→ `save_document` 一次发布（同快照先落 `.tex`；save 只做终态复核）。**图表三件套与逐条编号纪律**见 `文档/论文写作.md`。
8. **数学验证（论文生成后、评审前）**：独立检查已生成论文与真实结果的一致性，逐项核对，不通过回 Step 7 修正。机器部分：`python tools/project_ops/nine_step_verification.py <项目> --docx <论文.docx>`（硬错误必须 0），其余验证项按《文档/论文评审.md》验证项映射逐项过。此步不改论文不重新建模，只查一致性。
...
**步间交接核验（每步以机器结果放行，漏一项即回步）**：

| 交接 | 放行条件 |
|---|---|
| 0→1 | `self_check.py` exit 0 |
| 1→2 | `files/` 全量读取完成，附件对象清单已逐项视觉检查 |
| 2→3 | Problem Card 逐小问填写完整（schema 结构齐全） |
| 3→4 | `results/数据/文献检索.csv` 每行 citation_ready=true（未触发检索则跳过） |
| 4→5 | Model Contract 填写完整（validation/fallback 非空） |
| 5→6 | `code/Q<序号>.py` 全部落地且单问可一键运行 |
| 6→7 | 健全性检查 6 条过 + `validate_figure` 0 FAIL + 无英文标签 + 共享导出（无 P21） |
| 7→8 | `validate_paper_structure` 0 硬错误 0 预警（`save_document` 已发布） |
| 8→9 | `nine_step_verification` 硬错误 0 + 验证项映射表逐项过 |
| 9→交付 | 评审 P0/P1 清零 + `project_audit.py`/`self_check.py` exit 0 |
...
**FORCE 模式下仍可做的事**：对被锁模型的关键假设做灵敏度/消融验证、检验参数合理性、在评审文件中标注局限。不可擅自换模型或拼接其他方案。**PREFER 模式下**用户建议模型与自主候选地位平等；未采用时在评审文件中说明原因。**Tournament 操作细则**：实验记录落 `results/数据/tournament_log.md`，评分公式与预算约束见 `知识库/建模增强/冠军挑战者建模流程.md §11`。
...
4. **表格形态**：正文表默认三线表，大型数据表允许闭合方框表；两线表、带竖线的网格表一律拒存。形态细则见 `文档/样式统一规定.md §七`。
5. **模型建立节公式**：标题含"建立/建模"的小节必须 ≥1 个 oMath 公式，空缺即拒。
6. **结果与版式规则**：统一执行 `文档/论文写作.md`；机器校验统一执行 `tools/docx/core/paper_format.py`，运行结果统一执行 `tools/docx/core/structure_validation.py`。
...
│   ├── 数据/                  # 当前运行的数据 csv/json（xlsx/csv/md）
│   ├── 图片/                  # 当前运行的图片
├── 完整论文.docx
