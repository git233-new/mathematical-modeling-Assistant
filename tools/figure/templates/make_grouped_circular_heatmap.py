...
TRAITS_OUTER_TO_INNER = [
    TraitSpec("失眠", "#51448a", "#e7e4f2"),
    TraitSpec("睡眠时长", "#606766", "#ededeb"),
    TraitSpec("长睡眠", "#4e9568", "#e2f0e4"),
    TraitSpec("短睡眠", "#bd454c", "#f5dddd"),
    TraitSpec("睡眠时型", "#7b54b9", "#ece3f6"),
    TraitSpec("晨型", "#3d719b", "#e3edf5"),
    TraitSpec("午睡频率", "#e58a50", "#f9e4d5"),
    TraitSpec("嗜睡", "#5d6a67", "#e3e9e8"),
]


PAIR_GROUPS = [
    PairGroup("睡眠特征→皮层表面积", "#a9d9e8", 10),
    PairGroup("睡眠特征→皮层厚度", "#1f79b5", 10),
    PairGroup("睡眠特征→皮层下体积", "#9ee48e", 10),
    PairGroup("睡眠特征→纵向变化", "#22a33a", 10),
    PairGroup("皮层表面积→睡眠特征", "#f28a9e", 12),
    PairGroup("皮层厚度→睡眠特征", "#f7ba67", 12),
    PairGroup("皮层下体积→睡眠特征", "#c8b3d8", 14),
    PairGroup("纵向变化→睡眠特征", "#7b3db6", 14),
]
...
def add_center_legend(fig: plt.Figure) -> None:
    ...
    ax.set_zorder(4)
    ax.axis("off")
    ax.text(0.00, 1.02, "暴露-结局配对", fontsize=10, fontweight="bold", ha="left", va="bottom")
