...
def draw_raincloud(ax: plt.Axes, grouped_values: dict[str, np.ndarray], metric: str, show_ylabels: bool) -> None:
    ...
    ax.set_title(cfg["title"], fontsize=9, fontweight="bold", pad=5)
    ...
    if cfg["unit"]:
        ax.text(1.01, -0.05, cfg["unit"], transform=ax.transAxes, ha="left", va="top", fontsize=8)
...
def add_city_and_legend_panel(ax: plt.Axes) -> None:
    ...
    ax.text(0.12, y0 - 0.050, "中位数线", va="center", fontsize=8)
    ax.scatter(0.045, y0 - 0.105, marker="^", s=16, color="#d44d5d", edgecolor="white", linewidth=0.3)
    ax.text(0.12, y0 - 0.105, "均值", va="center", fontsize=8)
    ...
    ax.text(0.12, y0 - 0.160, "1.5倍四分位距范围", va="center", fontsize=8)
    ax.plot([0.00, 0.09], [y0 - 0.215, y0 - 0.215], color="#2f6791", lw=0.9)
    ax.text(0.12, y0 - 0.215, "均值连接线", va="center", fontsize=8)
...
def draw_panel_a(ax: plt.Axes) -> None:
    ...
    ax.set_xlabel("公园数量", fontsize=9)
    ax.set_title("各城市公园数量", fontsize=9, fontweight="bold", pad=5)
