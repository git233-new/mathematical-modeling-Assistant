...
def draw_model_panel(fig: plt.Figure, slot, panel: ModelPanel, seed: int) -> None:
    ...
    ax_top.set_title(f"{panel.name}：预测值 vs 真实值（直方图+核密度）", fontweight="bold", pad=5)
...
def draw_scatter_panel(
    ...
    ax.set_xlim(-5, 110)
    ax.set_ylim(-5, 112)
    ax.set_xlabel("实际值", fontweight="bold", labelpad=2)
    ax.set_ylabel("预测值", fontweight="bold", labelpad=2)
    ax.tick_params()
    ax.legend(loc="upper left", handletextpad=0.5, borderaxespad=0.45)
    ax.text(
        0.28,
        0.035,
        panel.metric_text,
