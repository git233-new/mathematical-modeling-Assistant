...
def main() -> None:
    ...
    src = skill_root / "templates" / SCRIPT_MAP[template_id]
    if not src.exists():
    ...
    for runtime_path in sync_shared_runtime(scripts_dir, overwrite=args.overwrite):
        print(f"Copied shared figure runtime: {runtime_path}")
