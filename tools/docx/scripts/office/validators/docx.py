...
class DOCXSchemaValidator(BaseSchemaValidator):
    ...
    @staticmethod
    def _durable_id_base(xml_file) -> int:
        """numbering.xml 的 durableId 按十进制解析，其余 XML 按十六进制。"""
        return 10 if xml_file.name == "numbering.xml" else 16
            ...
            try:
                for elem in lxml.etree.parse(str(xml_file), _SAFE_XML_PARSER).iter():
                    ...
                    if val := elem.get(durable_id_attr):
                        base = self._durable_id_base(xml_file)
                        if base == 10:
                            try:
                                if self._parse_id_value(val, base=base) >= 0x7FFFFFFF:
                                    errors.append(
                        ...
                        elif self._parse_id_value(val, base=base) >= 0x7FFFFFFF:
                            errors.append(
                                f"  {xml_file.name}:{elem.sourceline}: "
                                f"durableId={val} >= 0x7FFFFFFF"
                            )
            except Exception as exc:
                logger.warning("ID 约束校验失败，跳过该文件: %s", exc)
        ...
        for xml_file in self.xml_files:
            try:
                ...
                for elem in dom.getElementsByTagName("*"):
                    ...
                    try:
                        needs_repair = (
                            self._parse_id_value(durable_id, base=base) >= 0x7FFFFFFF
                        )
                    except ValueError:
                        needs_repair = True
