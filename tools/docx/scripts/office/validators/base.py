...
from tools.common.io_utils import safe_extract_zip, safe_xml_parser

import logging
logger = logging.getLogger(__name__)

_SAFE_XML_PARSER = safe_xml_parser()
...
class BaseSchemaValidator:
    ...
    def _report_validation(self, errors, *, violation_label, pass_message, extra_hint=None):
        """统一校验输出：有错打印 FAILED+明细（可选附加提示），无错打印 PASSED。"""
        if errors:
            print(f"FAILED - Found {len(errors)} {violation_label} violations:")
            for error in errors:
                print(error)
            if extra_hint:
                print(extra_hint)
    ...
    def repair_word_style_child_order(self) -> int:
        ...
        parser = lxml.etree.XMLParser(resolve_entities=False, no_network=True, load_dtd=False, remove_blank_text=False)
        try:
    ...
    def repair_whitespace_preservation(self) -> int:
        ...
        for xml_file in self.xml_files:
            try:
                ...
                logger.warning("XML 修复失败，跳过该文件: %s", exc)
    ...
    def validate_unique_ids(self):
        ...
        return self._report_validation(errors, violation_label="ID uniqueness", pass_message="All required IDs are unique")
    ...
    def validate_file_references(self):
        ...
        return self._report_validation(
            errors,
            violation_label="relationship validation",
            pass_message="All references are valid and all files are properly referenced",
            extra_hint="CRITICAL: These errors will cause the document to appear corrupt. Broken references MUST be fixed, and unreferenced files MUST be referenced or removed.",
        )
    ...
    def validate_all_relationship_ids(self):
        ...
        return self._report_validation(
            errors,
            violation_label="relationship ID reference",
            pass_message="All relationship ID references are valid",
    ...
    def validate_content_types(self):
        ...
        return self._report_validation(errors, violation_label="content type declaration", pass_message="All content files are properly declared in [Content_Types].xml")
