"""
Command line tool to validate Office document XML files against XSD schemas and tracked changes.

Usage:
    python validate.py <path> [--original <original_file>] [--auto-repair] [--author NAME]

The first argument can be either:
- An unpacked directory containing the Office document XML files
- A packed Office file (.docx/.pptx/.xlsx) which will be unpacked to a temp directory

Auto-repair fixes:
- paraId/durableId values that exceed OOXML limits
- Missing xml:space="preserve" on w:t elements with whitespace
"""

import argparse
import sys
import tempfile
import zipfile
from pathlib import Path

_ROOT_DIR = next((p for p in Path(__file__).resolve().parents if (p / "tools" / "common").is_dir()), None)
if _ROOT_DIR is not None and str(_ROOT_DIR) not in sys.path:
    sys.path.insert(0, str(_ROOT_DIR))
from tools.common.io_utils import safe_extract_zip
sys.path.insert(0, str(Path(__file__).resolve().parent))
from validators import DOCXSchemaValidator, PPTXSchemaValidator, RedliningValidator


def _require(condition: bool, message: str) -> None:
    if not condition:
        raise ValueError(message)


def validate_file(path, original=None, verbose=False, auto_repair=False, author="Claude") -> bool:
    path = Path(path)
    _require(path.exists(), f"Error: {path} does not exist")

    original_file = None
    if original:
        original_file = Path(original)
        _require(original_file.is_file(), f"Error: {original_file} is not a file")
        _require(original_file.suffix.lower() in [".docx", ".pptx", ".xlsx"],
            f"Error: {original_file} must be a .docx, .pptx, or .xlsx file"
        )

    file_extension = (original_file or path).suffix.lower()
    _require(file_extension in [".docx", ".pptx", ".xlsx"],
        f"Error: Cannot determine file type from {path}. Use --original or provide a .docx/.pptx/.xlsx file."
    )

    temp_dir = None
    if path.is_file() and path.suffix.lower() in [".docx", ".pptx", ".xlsx"]:
        temp_dir = tempfile.TemporaryDirectory(prefix="office-validate-")
        with zipfile.ZipFile(path, "r") as zf:
            safe_extract_zip(zf, temp_dir.name)
        unpacked_dir = Path(temp_dir.name)
    else:
        _require(path.is_dir(), f"Error: {path} is not a directory or Office file")
        unpacked_dir = path

    try:
        match file_extension:
            case ".docx":
                validators = [
                    DOCXSchemaValidator(unpacked_dir, original_file, verbose=verbose),
                ]
                if original_file:
                    validators.append(
                        RedliningValidator(unpacked_dir, original_file, verbose=verbose, author=author)
                    )
            case ".pptx":
                validators = [
                    PPTXSchemaValidator(unpacked_dir, original_file, verbose=verbose),
                ]
            case _:
                print(f"Error: Validation not supported for file type {file_extension}")
                return False

        if auto_repair:
            total_repairs = sum(v.repair() for v in validators)
            if total_repairs:
                print(f"Auto-repaired {total_repairs} issue(s)")

        success = all(v.validate() for v in validators)
        if success:
            print("All validations PASSED!")
        return success
    finally:
        if temp_dir is not None:
            temp_dir.cleanup()


def main():
    parser = argparse.ArgumentParser(description="Validate Office document XML files")
    parser.add_argument(
        "path",
        help="Path to unpacked directory or packed Office file (.docx/.pptx/.xlsx)",
    )
    parser.add_argument(
        "--original",
        required=False,
        default=None,
        help="Path to original file (.docx/.pptx/.xlsx). If omitted, all XSD errors are reported and redlining validation is skipped.",
    )
    parser.add_argument(
        "-v",
        "--verbose",
        action="store_true",
        help="Enable verbose output",
    )
    parser.add_argument(
        "--auto-repair",
        action="store_true",
        help="Automatically repair common issues (hex IDs, whitespace preservation)",
    )
    parser.add_argument(
        "--author",
        default="Claude",
        help="Author name for redlining validation (default: Claude)",
    )
    args = parser.parse_args()
    success = validate_file(args.path, args.original, args.verbose, args.auto_repair, args.author)
    sys.exit(0 if success else 1)


if __name__ == "__main__":
    main()
