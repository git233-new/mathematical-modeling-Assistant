...
import tempfile
import zipfile
from tools.docx.scripts.office.soffice import get_soffice_env
...
def _setup_libreoffice_macro(profile: Path) -> bool:
    ...
    try:
        try:
            ...
            logger.warning("未找到 LibreOffice 可执行文件 soffice")
            return False
        except subprocess.TimeoutExpired:
            logger.warning("LibreOffice 初始化超时")
            return False
...
def accept_changes(input_file: str, output_file: str) -> tuple[Path | None, str | None]:
...
if __name__ == "__main__":
    ...
    result, message = accept_changes(args.input_file, args.output_file)
    if result is not None:
        print(f"已接受全部修订: {args.input_file} -> {args.output_file}")
    else:
        print(message)
        raise SystemExit(1)
