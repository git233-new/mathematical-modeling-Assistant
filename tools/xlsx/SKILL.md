---
name: mathmodel-xlsx
description: 数学建模流程中的 Excel 工具：读取、创建、修改和验证 XLSX，支持模板保留、公式重算和错误检查。
license: Proprietary. See tools/docx/LICENSE.txt for complete terms
---
...
1. **行数门禁**：读取后必须核对首行、末行、原始工作表有效行数和题目声明的记录数；行数不符立即报错，不得静默继续建模。
2. **表头显式声明**：第一行就是数据时必须显式使用 `header=None` 或 `header=False`，禁止依赖 `pandas.read_excel()` 默认 `header=0` 猜测。
3. **模板保护**：使用模板时用 `openpyxl.load_workbook()` 打开、写入指定单元格后另存新文件；不删除未知工作表、命名区域、公式或验证规则。
4. **公式重算成功门禁**：修改含公式的工作簿后必须执行 `recalc.py` 隔离重算；工具仅在成功后原子替换目标文件，超时/失败/未生成输出时保留原文件，并报告错误而非静默接受旧值。
5. **错误值零容忍**：交付前确认无 `#VALUE!`、`#DIV/0!`、`#REF!`、`#NAME?`、`#NULL!`、`#NUM!`、`#N/A` 任一错误值。
