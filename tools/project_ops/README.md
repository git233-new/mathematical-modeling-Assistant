...
| `project_cleanup.py` | 论文生成成功后清理临时脚本、旧版论文和构建中间文件；CLI `--apply` 与库函数 `cleanup_after_delivery` 走同一守卫（`results/`、`code/`、`完整论文.docx` 白名单 + SKILL_ROOT 只读校验），默认仅预览，`论文模板.docx` 在预览中可见 |
